...
def T_2(h):
    return 11.7 + 5.9*(h + 8) + (-2.65)*(h + 8)*(h + 7)
# x,y are 1d- arrays
def newtonDD(x, y):
    n = len(x)
    if (len(y) != n): exit(1)
    ...
    for i in range(n):
        pyp.plot(x[i], y[i], 'k*')
    pyp.xlabel('Depth (m)')
    pyp.ylabel('Temperature (°C)')
    pyp.xlim(-10, -2)
    pyp.ylim(10, 20)
    pyp.title("Newton DD interpolation")
    pyp.legend(["Newton DD interpolant", "Data points"], loc="best")
    pyp.scatter(-7.5, temp_estimate, color='red', label=f'Estimate at {-7.5}m')
    pyp.annotate(f'({h_value}, {temp_estimate:.4f})', (h_value, temp_estimate), textcoords="offset points", xytext=(-10, 10),
                 ha='center')
    pyp.show()
...
############################## MAIN #############################
years = np.array([-8, -7, -6])
production = np.array([11.7, 17.6, 18.2])
